(function () {
  ...
  var noConflict = function (uri, options) {
    ...
    options = options || {};
    options.sockjs = options.sockjs || {};
    options.sockjs.resource = 
          options.sockjs.resource || '//cdnjs.cloudflare.com/ajax/libs/sockjs-client/0.3.2/sockjs-min.js';
    options.sockjs.uri = options.sockjs.uri || '/__now.js';
    ...
    util = {
      ...
      emit: function (name, args) {
        if (util.hasProperty(util._events, name)) {
          util._events[name].slice(0).forEach( function(event) {
            event.apply(util, [].concat(args));
          });
        }
        return util;
      },
      ...
      watch: function (obj, label, fqn) {
        ...
        function setter(newVal) {
          if (val !== newVal && newVal !== fqnMap.get(fqn)) {
            ...
            socket.send({type: 'rv', data: toReplace});
          }
          return newVal;
        }
    ...
    lib = {
      ...
      constructRemoteFunction: function (fqn) {
        var remoteFn = function () {
          ...
          socket.send({type: 'rfc', data: {fqn: fqn, args: args}});
        };
      ...
      handleNewConnection: function (socket) {
        ...
        socket.onmessage = function(msg) {
            if(msg.type == 'rfc') {
                lib.remoteCall(msg.data);
                util.emit('rfc', msg.data);
            } else if(msg.type == 'rv') {
                lib.replaceVar(msg.data);
                util.emit('rv', msg.data);
            } else if(msg.type == 'del') {
                lib.deleteVar(msg.data);
                util.emit('del', msg.data);
            } else if(msg.type == 'rd') {
                if(++readied === 2) {
                    lib.serverReady();
                }
        ...
        if (obj && typeof obj === 'object') {
          ...
          if (keys && typeof keys === 'object') {
            ...
            if (toDelete.length > 0) {
              socket.send({type: 'del', data: toDelete});
            }
          }
        }
    ...
    var dependencies = [
      { key: 'sockjs', path: now.core.options.sockjs.resource }
    ];
    ...
    var scriptLoaded = function () {
      ...
      socket = new SockJS(now.core.options.sockjs.uri);
      now.core.socket = socket;
