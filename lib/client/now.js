(function () {
  ...
  var noConflict = function (uri, options) {
    ...
    options.sockjs.uri = options.sockjs.uri || '/_nowjs';
    ...
    var createSocket = function() {
      ...
      socket.emit = function(type, obj) {
          var tosend = {type: type}
          if(obj !== undefined) tosend.data = obj;
          return socket.send(JSON.stringify(tosend));
      }
    ...
    for (var i = 0, ii = dependencies.length; i < ii; i++) {
      ...
      fileref.setAttribute('src', dependencies[i]['path']);
